...
/maxx wire       # finish a plugin install: adds the statusline + hooks (a plugin can't write settings.json)
...
2. Run it:
   ...
   - Wire:      `node ~/.claude/skills/maxx/tracker.mjs wire`   (when the bar is missing after a `/plugin install`, or the card says something is not wired. Idempotent; refuses to touch an unparseable settings.json.)
